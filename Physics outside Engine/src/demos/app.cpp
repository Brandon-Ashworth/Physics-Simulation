...
{
    particleArray = new engine::Particle[particleCount];
...
void MassAggregateApplication::display()
{
    ...
    engine::ParticleWorld::Particles &particles = world.getParticles();
    for (engine::ParticleWorld::Particles::iterator p = particles.begin();
        p != particles.end();
        p++)
    {
        engine::Particle *particle = *p;
        const engine::Vector3 &pos = particle->getPosition();
        glPushMatrix();
...
void RigidBodyApplication::drawDebug()
{
    ...
    for (unsigned i = 0; i < cData.contactCount; i++)
    {
        ...
        engine::Vector3 vec = contacts[i].contactPoint;
