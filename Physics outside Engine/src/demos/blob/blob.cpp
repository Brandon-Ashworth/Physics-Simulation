 ...
 */
class Platform : public engine::ParticleContactGenerator
{
public:
    engine::Vector3 start;
    engine::Vector3 end;

    /**
     * Holds a pointer to the particles we're checking for collisions with.
     */
    engine::Particle *particles;

    virtual unsigned addContact(
        engine::ParticleContact *contact,
        unsigned limit
        ) const;
};

unsigned Platform::addContact(engine::ParticleContact *contact,
                              unsigned limit) const
{
    const static engine::real restitution = 0.0f;
    ...
    for (unsigned i = 0; i < BLOB_COUNT; i++)
    {
        ...
        engine::Vector3 toParticle = particles[i].getPosition() - start;
        engine::Vector3 lineDirection = end - start;
        engine::real projected = toParticle * lineDirection;
        engine::real platformSqLength = lineDirection.squareMagnitude();
        if (projected <= 0)
        {
 ...
 */
class BlobForceGenerator : public engine::ParticleForceGenerator
{
public:
    /**
    ...
    */
     ...
     * there is no force.
     */
    engine::real maxDistance;

    virtual void updateForce(
        engine::Particle *particle,
        engine::real duration
        );
};

void BlobForceGenerator::updateForce(engine::Particle *particle,
                                      engine::real duration)
{
    ...
    for (unsigned i = 0; i < BLOB_COUNT; i++)
    {
        ...
        engine::Vector3 separation =
            particles[i].getPosition() - particle->getPosition();
        separation.z = 0.0f;
        engine::real distance = separation.magnitude();
    ...
    {
        engine::real force = engine::real(joinCount / maxFloat) * floatHead;
        if (force > floatHead) force = floatHead;
        particle->addForce(engine::Vector3(0, force, 0));
    }
...
{
    engine::Particle *blobs;

    Platform *platforms;

    engine::ParticleWorld world;
...
world(PLATFORM_COUNT+BLOB_COUNT, PLATFORM_COUNT)
{
    ...
    blobs = new engine::Particle[BLOB_COUNT];
    engine::Random r;
    ...
    for (unsigned i = 0; i < PLATFORM_COUNT; i++)
    {
        platforms[i].start = cyclone::Vector3(
            engine::real(i%2)*10.0f - 5.0f,
            engine::real(i)*4.0f + ((i%2)?0.0f:2.0f),
            0);
        platforms[i].start.x += r.randomBinomial(2.0f);
        platforms[i].start.y += r.randomBinomial(2.0f);

        platforms[i].end = engine::Vector3(
            engine::real(i%2)*10.0f + 5.0f,
            engine::real(i)*4.0f + ((i%2)?2.0f:0.0f),
            0);
    ...
    engine::real fraction = (engine::real)1.0 / BLOB_COUNT;
    engine::Vector3 delta = p->end - p->start;
    for (unsigned i = 0; i < BLOB_COUNT; i++)
    {
        unsigned me = (i+BLOB_COUNT/2) % BLOB_COUNT;
        blobs[i].setPosition(
            p->start + delta * (engine::real(me)*0.8f*fraction+0.1f) +
            engine::Vector3(0, 1.0f+r.randomReal(), 0));

        blobs[i].setVelocity(0,0,0);
        blobs[i].setDamping(0.2f);
        blobs[i].setAcceleration(engine::Vector3::GRAVITY * 0.4f);
...
{
    engine::Vector3 pos = blobs[0].getPosition();
    ...
    for (unsigned i = 0; i < PLATFORM_COUNT; i++)
    {
        const engine::Vector3 &p0 = platforms[i].start;
        const engine::Vector3 &p1 = platforms[i].end;
    ...
    for (unsigned i = 0; i < BLOB_COUNT; i++)
    {
        const engine::Vector3 &p = blobs[i].getPosition();
        glPushMatrix();
    ...
    engine::Vector3 p = blobs[0].getPosition();
    engine::Vector3 v = blobs[0].getVelocity() * 0.05f;
...
void BlobDemo::update()
{
    ...
    blobs[0].addForce(engine::Vector3(xAxis, yAxis, 0)*10.0f);
