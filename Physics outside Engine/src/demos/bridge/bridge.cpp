...
{
    engine::ParticleCableConstraint *supports;
    engine::ParticleCable *cables;
    engine::ParticleRod *rods;

    engine::Vector3 massPos;
    engine::Vector3 massDisplayPos;
...
massPos(0,0,0.5f)
{
    ...
    for (unsigned i = 0; i < 12; i++)
    {
        ...
        particleArray[i].setPosition(
            engine::real(i/2)*2.0f-5.0f,
            4,
            engine::real(i%2)*2.0f-1.0f
            );
        particleArray[i].setVelocity(0,0,0);
        particleArray[i].setDamping(0.9f);
        particleArray[i].setAcceleration(engine::Vector3::GRAVITY);
        particleArray[i].clearAccumulator();
    }

    // Add the links
    cables = new engine::ParticleCable[CABLE_COUNT];
    ...
    supports = new engine::ParticleCableConstraint[SUPPORT_COUNT];
    for (unsigned i = 0; i < SUPPORT_COUNT; i++)
    {
        supports[i].particle = particleArray+i;
        supports[i].anchor = engine::Vector3(
            engine::real(i/2)*2.2f-5.5f,
            6,
            engine::real(i%2)*1.6f-0.8f
            );
        if (i < 6) supports[i].maxLength = engine::real(i/2)*0.5f + 3.0f;
        else supports[i].maxLength = 5.5f - engine::real(i/2)*0.5f;
        supports[i].restitution = 0.5f;
        world.getContactGenerators().push_back(&supports[i]);
    }

    rods = new engine::ParticleRod[ROD_COUNT];
...
void BridgeDemo::updateAdditionalMass()
{
    ...
    int x = int(massPos.x);
    engine::real xp = real_fmod(massPos.x, engine::real(1.0f));
    if (x < 0)
    {
        x = 0;
        xp = 0;
    ...
    int z = int(massPos.z);
    engine::real zp = real_fmod(massPos.z, engine::real(1.0f));
    if (z < 0)
    {
        z = 0;
        zp = 0;
...
void BridgeDemo::display()
{
    ...
    {
        engine::Particle **particles = rods[i].particle;
        const engine::Vector3 &p0 = particles[0]->getPosition();
        const engine::Vector3 &p1 = particles[1]->getPosition();
    ...
    {
        engine::Particle **particles = cables[i].particle;
        const engine::Vector3 &p0 = particles[0]->getPosition();
        const engine::Vector3 &p1 = particles[1]->getPosition();
        glVertex3f(p0.x, p0.y, p0.z);
        glVertex3f(p1.x, p1.y, p1.z);
    ...
    for (unsigned i = 0; i < SUPPORT_COUNT; i++)
    {
        const engine::Vector3 &p0 = supports[i].particle->getPosition();
        const engine::Vector3 &p1 = supports[i].anchor;
        glVertex3f(p0.x, p0.y, p0.z);
        glVertex3f(p1.x, p1.y, p1.z);
    }
    glEnd();
