...
static engine::Random crandom;

/**
 * Fireworks are particles, with additional data for rendering and
 * evolution.
 */
class Firework : public engine::Particle
{
public:
     ...
     * removed.
     */
    bool update(engine::real duration)
    {
...
struct FireworkRule
{
    ...
    engine::Vector3 minVelocity;

    /** The maximum relative velocity of this firework. */
    engine::Vector3 maxVelocity;
     ...
     */
    void setParameters(unsigned type, engine::real minAge, engine::real maxAge,
        const engine::Vector3 &minVelocity, const engine::Vector3 &maxVelocity,
        engine::real damping)
    {
    ...
    void create(Firework *firework, const Firework *parent = NULL) const
    {
        ...
        firework->setAcceleration(engine::Vector3::GRAVITY);
...
void FireworksDemo::initFireworkRules()
{
    ...
    rules[0].setParameters(
        1, // type
        0.5f, 1.4f, // age range
        engine::Vector3(-5, 25, -5), // min velocity
        engine::Vector3(5, 28, 5), // max velocity
        0.1 // damping
        );
    ...
    rules[1].setParameters(
        2, // type
        0.5f, 1.0f, // age range
        engine::Vector3(-5, 10, -5), // min velocity
        engine::Vector3(5, 20, 5), // max velocity
        0.8 // damping
        );
    ...
    rules[2].setParameters(
        3, // type
        0.5f, 1.5f, // age range
        engine::Vector3(-5, -5, -5), // min velocity
        engine::Vector3(5, 5, 5), // max velocity
        0.1 // damping
        );
    ...
    rules[3].setParameters(
        ...
        engine::Vector3(-20, 5, -5), // min velocity
        engine::Vector3(20, 5, 5), // max velocity
        0.2 // damping
        );
    ...
    rules[4].setParameters(
        5, // type
        0.5f, 1.0f, // age range
        engine::Vector3(-20, 2, -5), // min velocity
        engine::Vector3(20, 18, 5), // max velocity
        0.01 // damping
        );
    ...
    rules[5].setParameters(
        ...
        3, 5, // age range
        engine::Vector3(-5, 5, -5), // min velocity
        engine::Vector3(5, 10, 5), // max velocity
        0.95 // damping
        );
    ...
    rules[6].setParameters(
        7, // type
        4, 5, // age range
        engine::Vector3(-5, 50, -5), // min velocity
        engine::Vector3(5, 60, 5), // max velocity
        0.01 // damping
        );
    ...
    rules[7].setParameters(
        8, // type
        0.25f, 0.5f, // age range
        engine::Vector3(-1, -1, -1), // min velocity
        engine::Vector3(1, 1, 1), // max velocity
        0.01 // damping
        );
    ...
    rules[8].setParameters(
        9, // type
        3, 5, // age range
        engine::Vector3(-15, 10, -5), // min velocity
        engine::Vector3(15, 15, 5), // max velocity
        0.95 // damping
        );
...
{
    const static engine::real size = 0.1f;
        ...
        firework++)
    {
        ...
        if (firework->type > 0)
        {
            ...
            const engine::Vector3 &pos = firework->getPosition();
