...
{
    engine::AeroControl left_wing;
    engine::AeroControl right_wing;
    engine::AeroControl rudder;
    engine::Aero tail;
    engine::RigidBody aircraft;
    engine::ForceRegistry registry;

    engine::Vector3 windspeed;
...
right_wing(engine::Matrix3(0,0,0, -1,-0.5f,0, 0,0,0),
           engine::Matrix3(0,0,0, -0.995f,-0.5f,0, 0,0,0),
           engine::Matrix3(0,0,0, -1.005f,-0.5f,0, 0,0,0),
           engine::Vector3(-1.0f, 0.0f, 2.0f), &windspeed),

left_wing(engine::Matrix3(0,0,0, -1,-0.5f,0, 0,0,0),
          engine::Matrix3(0,0,0, -0.995f,-0.5f,0, 0,0,0),
          engine::Matrix3(0,0,0, -1.005f,-0.5f,0, 0,0,0),
          engine::Vector3(-1.0f, 0.0f, -2.0f), &windspeed),

rudder(engine::Matrix3(0,0,0, 0,0,0, 0,0,0),
       engine::Matrix3(0,0,0, 0,0,0, 0.01f,0,0),
       engine::Matrix3(0,0,0, 0,0,0, -0.01f,0,0),
       engine::Vector3(2.0f, 0.5f, 0), &windspeed),

tail(engine::Matrix3(0,0,0, -1,-0.5f,0, 0,0,-0.1f),
     engine::Vector3(2.0f, 0, 0), &windspeed),
...
windspeed(0,0,0)
{
    ...
    engine::Matrix3 it;
    it.setBlockInertiaTensor(engine::Vector3(2,1,1), 1);
    aircraft.setInertiaTensor(it);

    aircraft.setDamping(0.8f, 0.8f);

    aircraft.setAcceleration(engine::Vector3::GRAVITY);
...
void FlightSimDemo::display()
{
    ...
    engine::Vector3 pos = aircraft.getPosition();
    engine::Vector3 offset(4.0f+aircraft.getVelocity().magnitude(), 0, 0);
    ...
    engine::Matrix4 transform = aircraft.getTransform();
...
void FlightSimDemo::update()
{
    ...
    engine::Vector3 propulsion(-10.0f, 0, 0);
