...
engine::Random global_random;

class Block : public engine::CollisionBox
{
public:
    bool exists;
     ...
     */
    void calculateMassProperties(engine::real invDensity)
    {
        ...
        if (invDensity <= 0)
        {
            ...
            body->setInverseInertiaTensor(engine::Matrix3());
        }
        else
        {
            // Otherwise we need to calculate the mass
            engine::real volume = halfSize.magnitude() * 2.0;
            engine::real mass = volume / invDensity;
     ...
     */
    void divideBlock(const engine::Contact& contact,
        Block* target, Block* blocks)
    {
        ...
        engine::Vector3 normal = contact.contactNormal;
        engine::RigidBody *body = contact.body[0];
        if (body != target->body)
        {
            normal.invert();
            body = contact.body[1];
        ...
        // and its direction.
        engine::Vector3 point = body->getPointInLocalSpace(contact.contactPoint);
        ...
        engine::Vector3 size = target->halfSize;

        // Take a copy also of the body's other data.
        engine::RigidBody tempBody;
        ...
        for (unsigned i = 0; i < 8; i++)
        {
            ...
            // local coordinates.
            engine::Vector3 halfSize = (max - min) * 0.5f;
            engine::Vector3 newPos = halfSize + min;

            // Convert the origin to world coordinates.
            newPos = tempBody.getPointInWorldSpace(newPos);

            // Work out the direction to the impact.
            engine::Vector3 direction = newPos - contact.contactPoint;
            direction.normalise();
            ...
            blocks[i].body->setAcceleration(engine::Vector3::GRAVITY);
            blocks[i].body->clearAccumulators();
            blocks[i].body->calculateDerivedData();
            blocks[i].offset = engine::Matrix4();
...
{
    // Create the ball.
    ball.body = new engine::RigidBody();
    ball.radius = 0.25f;
    ...
    ball.body->setAcceleration(engine::Vector3::GRAVITY);
...
void FractureDemo::generateContacts()
{
    ...
    cData.restitution = (engine::real)0.2;
    ...
    for (Block *block = blocks; block < blocks+MAX_BLOCKS; block++)
    {
        ...
        engine::CollisionDetector::boxAndHalfSpace(*block, plane, &cData);

        if (ball_active)
        {
            // And with the sphere
            if (!cData.hasMoreContacts()) return;
            if (engine::CollisionDetector::boxAndSphere(*block, ball, &cData))
            {
                hit = true;
        ...
        for (Block *other = block+1; other < blocks+MAX_BLOCKS; other++)
        {
            ...
            if (!cData.hasMoreContacts()) return;
            engine::CollisionDetector::boxAndBox(*block, *other, &cData);
        }
    }
...
void FractureDemo::reset()
{
    ...
    // Set the first block
    blocks[0].halfSize = engine::Vector3(4,4,4);
    ...
    blocks[0].body->setAcceleration(engine::Vector3::GRAVITY);
...
void FractureDemo::updateObjects(engine::real duration)
{
    for (Block *block = blocks; block < blocks+MAX_BLOCKS; block++)
    {
        if (block->exists)
...
void FractureDemo::display()
{
    ...
    if (ball_active)
    {
        ...
        glPushMatrix();
        engine::Vector3 pos = ball.body->getPosition();
