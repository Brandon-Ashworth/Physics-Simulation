...
class AmmoRound : public engine::CollisionSphere
{
public:
    ShotType type;
    unsigned startTime;
    ...
    void setState(ShotType shotType)
    {
        ...
        engine::real coeff = 0.4f*body->getMass()*radius*radius;
...
{
    return "engine > Big Ballistic Demo";
}
...
void BigBallisticDemo::updateObjects(engine::real duration)
{
...
void BigBallisticDemo::generateContacts()
{
    ...
    cData.restitution = (engine::real)0.1;
    ...
    for (Box *box = boxData; box < boxData+boxes; box++)
    {
        ...
        for (AmmoRound *shot = ammo; shot < ammo+ammoRounds; shot++)
        {
            if (shot->type != UNUSED)
            {
                ...
                if (engine::CollisionDetector::boxAndSphere(*box, *shot, &cData))
                {
                    shot->type = UNUSED;
                }
            }
